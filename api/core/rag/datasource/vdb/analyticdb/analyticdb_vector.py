...
class AnalyticdbVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> AnalyticdbVector:
        ...
        if nexusai_config.ANALYTICDB_HOST is None:
            # implemented through OpenAPI
            apiConfig = AnalyticdbVectorOpenAPIConfig(
                access_key_id=nexusai_config.ANALYTICDB_KEY_ID or "",
                access_key_secret=nexusai_config.ANALYTICDB_KEY_SECRET or "",
                region_id=nexusai_config.ANALYTICDB_REGION_ID or "",
                instance_id=nexusai_config.ANALYTICDB_INSTANCE_ID or "",
                account=nexusai_config.ANALYTICDB_ACCOUNT or "",
                account_password=nexusai_config.ANALYTICDB_PASSWORD or "",
                namespace=nexusai_config.ANALYTICDB_NAMESPACE or "",
                namespace_password=nexusai_config.ANALYTICDB_NAMESPACE_PASSWORD,
            )
            sqlConfig = None
        else:
            # implemented through sql
            sqlConfig = AnalyticdbVectorBySqlConfig(
                host=nexusai_config.ANALYTICDB_HOST,
                port=nexusai_config.ANALYTICDB_PORT,
                account=nexusai_config.ANALYTICDB_ACCOUNT or "",
                account_password=nexusai_config.ANALYTICDB_PASSWORD or "",
                min_connection=nexusai_config.ANALYTICDB_MIN_CONNECTION,
                max_connection=nexusai_config.ANALYTICDB_MAX_CONNECTION,
                namespace=nexusai_config.ANALYTICDB_NAMESPACE or "",
            )
            apiConfig = None
