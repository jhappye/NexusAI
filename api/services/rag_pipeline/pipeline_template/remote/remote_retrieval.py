...
class RemotePipelineTemplateRetrieval(PipelineTemplateRetrievalBase):
    """
    Retrieval recommended app from nexusai official
    """

    def get_pipeline_template_detail(self, template_id: str) -> dict | None:
        result: dict | None
        try:
            result = self.fetch_pipeline_template_detail_from_nexusai_official(template_id)
        except Exception as e:
            logger.warning("fetch recommended app detail from nexusai official failed: %r, switch to database.", e)
    ...
    def get_pipeline_templates(self, language: str) -> dict:
        try:
            result = self.fetch_pipeline_templates_from_nexusai_official(language)
        except Exception as e:
            logger.warning("fetch pipeline templates from nexusai official failed: %r, switch to database.", e)
    ...
    @classmethod
    def fetch_pipeline_template_detail_from_nexusai_official(cls, template_id: str) -> dict:
        """
        Fetch pipeline template detail from nexusai official.
        ...
        """
        domain = nexusai_config.HOSTED_FETCH_PIPELINE_TEMPLATES_REMOTE_DOMAIN
    ...
    @classmethod
    def fetch_pipeline_templates_from_nexusai_official(cls, language: str) -> dict:
        """
        Fetch pipeline templates from nexusai official.
