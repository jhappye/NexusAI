...
class FeedbackService:
    ...
    def _export_csv(data, app_id):
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename=nexusai_feedback_export_{app_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
    ...
    def _export_json(data, app_id):
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename=nexusai_feedback_export_{app_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
